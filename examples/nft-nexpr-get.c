...
static int nexpr_cb(const struct nlmsghdr *nlh, void *data)
{
	...
	if (nftnl_nexpr_nlmsg_parse(nlh, t) < 0) {
		perror("nftnl_nexpr_nlmsg_parse");
		goto err_free;
	}

	nftnl_nexpr_snprintf(buf, sizeof(buf), t, *type, 0);
	printf("%s\n", buf);
...
int main(int argc, char *argv[])
{
	...
	struct nftnl_nexpr *t = NULL;
	int ret;
	uint32_t type = NFTNL_OUTPUT_DEFAULT;

	if (argc < 2 || argc > 5) {
		fprintf(stderr, "%s <family> <table> [<nexpr>] [<default|xml|json>]\n",
			argv[0]);
		return EXIT_FAILURE;
	}
	...
	seq = time(NULL);
	if (argc < 4) {
		nlh = nftnl_nlmsg_build_hdr(buf, NFT_MSG_GETNEXPR, family,
					    NLM_F_DUMP, seq);
		if (argc == 3) {
			nftnl_nexpr_set(t, NFTNL_NEXPR_TABLE, argv[2]);
		...
		nftnl_nexpr_set(t, NFTNL_NEXPR_TABLE, argv[2]);
		nftnl_nexpr_set(t, NFTNL_NEXPR_NAME, argv[3]);
	...
	while (ret > 0) {
		ret = mnl_cb_run(buf, ret, seq, portid, nexpr_cb, &type);
		if (ret <= 0)
			break;
		ret = mnl_socket_recvfrom(nl, buf, sizeof(buf));
	}
